...
class Markup {
  ...
  static vkPayToGroup(color = 'default', groupId = 182657745){
    return {
      action: {
        type: 'vkpay',
        payload: "Задонатить разработчикам",
        hash: 'action=transfer-to-group&group_id=' + groupId
      },
      color
    }
  }

  static openApp(app_id, label, color = 'default'){
    return {
      action: {
        type: 'open_app',
        app_id: app_id,
